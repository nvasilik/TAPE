...
from tape.configs import to_lower
from .dataset import Dataset
from ._image_dataset import ImageDataset
from .mocap_dataset import MoCapDataset,MoCapDatasetVIBE
...
class TAPEDataModule(pl.LightningDataModule):
    ...
    def __init__(self, cfg: CfgNode, dataset_cfg: CfgNode) -> None:
        """
        Initialize LightningDataModule for TAPE training
        Args:
            cfg (CfgNode): Config file as a yacs CfgNode containing necessary dataset info.
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        Args:
            cfg (CfgNode): Config file as a yacs CfgNode containing necessary dataset info.
        """
        self.train_dataset = MixedDataset(self.cfg, self.dataset_cfg, train=True)#TODO true
        ...
        if self.cfg.MODE.DISCRIMINATOR:
            ...
            self.mocap_dataset = MoCapDatasetVIBE()
    ...
    def train_dataloader(self) -> Dict:
        ...
        """
        #train,val = get_data_loaders(None)
        ...
        #import ipdb
        #ipdb.set_trace()
        #mocap_dataloader = torch.utils.data.DataLoader(self.mocap_dataset, self.cfg.TRAIN.NUM_TRAIN_SAMPLES * self.cfg.TRAIN.BATCH_SIZE, shuffle=True, drop_last=True, num_workers=self.cfg.GENERAL.NUM_WORKERS)
        if self.cfg.MODE.DISCRIMINATOR:
            mocap_dataloader = torch.utils.data.DataLoader(self.mocap_dataset, self.cfg.TRAIN.NUM_TRAIN_SAMPLES * self.cfg.TRAIN.BATCH_SIZE, shuffle=True, drop_last=True, num_workers=self.cfg.GENERAL.NUM_WORKERS)
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        """
        #train,val = get_data_loaders(None)
        #val_dataloader = torch.utils.data.DataLoader(self.val_dataset, self.cfg.TRAIN.BATCH_SIZE, shuffle=True, drop_last=True, num_workers=self.cfg.GENERAL.NUM_WORKERS)#TODO Shuffle true
        val_dataloader = torch.utils.data.DataLoader(self.val_dataset, 4, shuffle=False, drop_last=True, num_workers=self.cfg.GENERAL.NUM_WORKERS)#TODO Shuffle true
        #val_dataloader= val
...
class ProSkeletonDataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        Returns:
            torch.utils.data.DataLoader: Validation dataloader  
        """
        val_dataloader = torch.utils.data.DataLoader(self.val_dataset, self.cfg.TRAIN.BATCH_SIZE, shuffle=False, drop_last=True, num_workers=self.cfg.GENERAL.NUM_WORKERS)
        return val_dataloader
