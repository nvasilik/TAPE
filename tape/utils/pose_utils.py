...
from lib.utils.eval_utils import compute_accel, compute_error_accel, batch_compute_similarity_transform_torch, compute_error_verts, compute_errors
...
class Evaluator:
    ...
    def __init__(self,
        ...
        for metric in self.metrics:
            setattr(self, metric, np.zeros((dataset_length*50,)))
        self.counter = 0
    ...
    def __call__(self, output: Dict, batch: Dict, opt_output: Optional[Dict] = None):
        ...
        batch['keypoints_3d'] =batch['keypoints_3d'].reshape((batch_size,44,4))
        ...
        if opt_output is not None:
            ...
            opt_accel_err = np.zeros((len(opt_keypoints_3d,)))
            #import ipdb
            #ipdb.set_trace()
            opt_accel_err[1:-1] = compute_error_accel(joints_pred=opt_keypoints_3d.cpu().numpy(), joints_gt=gt_keypoints_3d[:,0,:].cpu().numpy())* m2mm

        #target_j3ds, target_theta = target_j3ds[:len(pred_j3ds)], target_theta[:len(pred_j3ds)]
        
        #accel_err = np.zeros((len(pred_keypoints_3d,)))
        #accel= np.zeros((len(pred_keypoints_3d,)))
        #accel = compute_accel(pred_keypoints_3d.reshape(batch_size * num_samples, -1 ,3).cpu().numpy())*m2mm
        #accel_err[1:-1] = compute_error_accel(joints_pred=pred_keypoints_3d.reshape(batch_size * num_samples, -1 ,3).cpu().numpy(), joints_gt=gt_keypoints_3d.reshape(batch_size * num_samples, -1 ,3).cpu().numpy() )* m2mm
        
        #if opt_output is None:
            #accel_err[1:-1] = compute_error_accel(joints_pred=pred_keypoints_3d.reshape(batch_size * num_samples, -1 ,3).cpu().numpy(), joints_gt=gt_keypoints_3d.reshape(batch_size * num_samples, -1 ,3).cpu().numpy() )* m2mm
        # The 0-th sample always corresponds to the mode
        if hasattr(self,'accel'):
            self.accel[self.counter:self.counter+batch_size-2]=accel
        if hasattr(self,'accel_err'):
            self.accel_err[self.counter:self.counter+batch_size] = accel_err
        if hasattr(self,'mpvpe'):
            pred_verts=output['pred_vertices'].reshape(batch_size*num_samples,-1,3)
            #betas=output['pred_smpl_params']['betas'].reshape(batch_size*num_samples,-1)
            dummy_cam = np.repeat(np.array([[1., 0., 0.]]), len(accel_err), axis=0)
            #target_theta =rotation_matrix_to_angle_axis(rot6d_to_rotmat(output['pred_pose_6d'][:,0,:])).reshape(batch_size*num_samples,72).cpu().numpy()
            target_theta=np.concatenate([dummy_cam,batch['smpl_params']['global_orient'].cpu().numpy(),batch['smpl_params']['body_pose'].cpu().numpy(),batch['smpl_params']['betas'].cpu().numpy()],axis=1).astype(np.float32)

            #target_theta = np.concatenate([dummy_cam, target_theta, betas.cpu().numpy()], axis=1).astype(np.float32)
            mpvpe = compute_error_verts(target_theta=target_theta, pred_verts=pred_verts) *1000
            self.mpvpe[self.counter:self.counter+batch_size]=mpvpe
        ...
        if hasattr(self,'opt_accel'):
            self.opt_accel[self.counter:self.counter+batch_size] = opt_accel_err
